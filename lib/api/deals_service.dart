...
class DealsService {
  ...
  Future<List<Deal>> getDeals(String organizationId, {String? stage}) async {
    final response = await _apiClient.get('/deals', queryParameters: {
  ...
  Future<Deal> updateDeal(String id, String organizationId, Map<String, dynamic> data) async {
    final response = await _apiClient.patch('/deals/$id', 
      data: data, 
  ...
  Future<void> deleteDeal(String id, String organizationId) async {
    await _apiClient.delete('/deals/$id', queryParameters: {'organizationId': organizationId});
  }
}
